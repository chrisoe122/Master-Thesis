...
SGBM_2<-function(X,Xa,KQ,KA,dt,Q,EX,fm,sigma1,lambda1,kp){
  ...
  #Under Q
  bmny<-matrix(data=NA, nrow=3, ncol=((TT-1)*2+1)) #col er tidsgrupperinger med 2 bundles i hver, så TT*2. row er antal parameter
  Cm<-matrix(data=NA, nrow=KQ, ncol=TT) 
  EQ<-matrix(data=NA, nrow=KQ, ncol=(TT+1))
  Dm<-matrix(data=NA, nrow=1, ncol=(TT)) #Upper bound for hver bundle
  ...
  for( p in 1:EX){
    ...
    for (j in 1:(1/dt-1)){ #ÆNDRER
      ...
      df1$quan<-ntile(df1[[4]], 2)#Number of groups
      CoefSG<-df1[,as.list(flm(X=cbind(1,V2,V3),y=V1)),by="quan"]
      bmny[,((time-3)*2+2):((time-2)*2+1)]<- t(CoefSG[order(quan),-1])
      ...
      Dm[,(time-1)]<-quantile(df1[[4]],probs = 0.5)
    ...
    EQ[,time-1]<-Re(Cm[,time-1]) #Ændret vær opmærksom!
    EQ[(Re(Cm[,time-1])!=Vny[,time-1]),]<-0 #Kan dette gøres hurtigere???? Mærkbart tungt
    time<- time-1
  }
  ...
  for( p in 1:EX){
    ...
    for (j in 1:(1/dt-1)){ #ÆNDRER
      ...
      dfb<-data.table(t(bmny[,((time-3)*2+2):((time-2)*2+1)]),qlabels)
  ...
  names(output)<-c("V0, MPFE, EPE, 100CVA", "V", "EQ", "EA", "PFE", "PFEq", "mEA", "mEQ", "Tid", "C", "bmny")
