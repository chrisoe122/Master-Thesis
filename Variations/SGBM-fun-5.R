...
SGBM_5<-function(X,Xa,KQ,KA,dt,Q,EX,fm,sigma1,lambda1,kp){
  ...
  #Under Q
  bmny<-matrix(data=NA, nrow=3, ncol=((TT-1)*5+1)) #col er tidsgrupperinger med 5 bundles i hver, så TT*5. row er antal parameter
  Cm<-matrix(data=NA, nrow=KQ, ncol=TT) 
  EQ<-matrix(data=NA, nrow=KQ, ncol=(TT+1))
  Dm<-matrix(data=NA, nrow=4, ncol=(TT)) #Upper bound for hver bundle
  ...
  for( p in 1:EX){
    ...
    for (j in 1:(1/dt-1)){ #ÆNDRER
      ...
      df1$quan<-ntile(df1[[4]], 5)#Number of groups
      CoefSG<-df1[,as.list(flm(X=cbind(1,V2,V3),y=V1)),by="quan"]
      bmny[,((time-3)*5+2):((time-2)*5+1)]<- t(CoefSG[order(quan),-1])
      ...
      Dm[,(time-1)]<-quantile(df1[[4]],probs = seq(0.2,0.8,0.2))
  ...
  for( p in 1:EX){
    ...
    for (j in 1:(1/dt-1)){ #ÆNDRER
      ...
      dfb<-data.table(t(bmny[,((time-3)*5+2):((time-2)*5+1)]),qlabels)
